...
local username = _G.obex_name == nil and 'Kenzo' or _G.obex_name:lower()
...
--kenzo.refs.aa.fake_yaw_limit                                    = ui.reference("AA", "Anti-aimbot angles", "Fake yaw limit")
...
--kenzo.refs.rage.double_tap, kenzo.refs.rage.double_tap_key      = ui.reference("RAGE", "Other", "Double tap")
--kenzo.refs.rage.sv_maxusrcmdprocessticks                        = ui.reference("MISC", "Settings", "sv_maxusrcmdprocessticks")
kenzo.refs.rage.holdaim                                         = ui.reference("MISC", "Settings", "sv_maxusrcmdprocessticks_holdaim")
--kenzo.refs.rage.force_bodyaim                                   = ui.reference("RAGE", "Other", "Force body aim")
--kenzo.refs.rage.prefer_bodyaim                                  = ui.reference("RAGE", "Other", "Prefer body aim")
kenzo.refs.rage.prefer_safepoint                                = ui.reference("RAGE", "Aimbot", "Prefer safe point")
kenzo.refs.rage.force_safepoint                                 = ui.reference("RAGE", "Aimbot", "Force safe point")
...
ui.set_visible(kenzo.refs.rage.holdaim, false)
...
for k, v in pairs(kenzo.antiaim.states) do
    ...
    kenzo.ui.aa.states[v].fake_yaw_limit_l      = kenzo.handlers.ui.new(  ui.new_slider("AA", "Anti-aimbot angles", "\a9ED8FFFF" .. v .. " -\aCDCDCDFF" .. " Fake yaw limit \a9ED8FFFFleft", 0, 60, 60, true, "°"), function() return show() and ui.get(kenzo.ui.aa.states[v].body_yaw) ~= "Off" and not hide_t0g end, true)
    kenzo.ui.aa.states[v].fake_yaw_limit_r      = kenzo.handlers.ui.new(  ui.new_slider("AA", "Anti-aimbot angles", "\a9ED8FFFF" .. v .. " -\aCDCDCDFF" .. " Fake yaw limit \a9ED8FFFFright", 0, 60, 60, true, "°"), function() return show() and ui.get(kenzo.ui.aa.states[v].body_yaw) ~= "Off" and not hide_t0g end, true)
...
client.set_event_callback("setup_command", function(cmd)
    ...
    --ui.set(kenzo.refs.aa.fake_yaw_limit, math.random(58,59))
end)
...
kenzo.handlers.aa.freestanding = function()
    ...
    --ui.set(kenzo.refs.aa.freestanding, freestanding and "Default" or "-")
end
...
--[[
	...
	if bodyyaw > 0 then
		--ui.set(kenzo.refs.aa.fake_yaw_limit, ui.get(kenzo.ui.aa.states[state].fake_yaw_limit_r))
	elseif bodyyaw < 0 then
--		ui.set(kenzo.refs.aa.fake_yaw_limit, ui.get(kenzo.ui.aa.states[state].fake_yaw_limit_l))
	end
end)
...
local inds = {
    ...
    {
        ref = kenzo.refs.rage.force_bodyaim,
        name = "BAIM"
    }
}
...
local function get_key_mode(ref)
  ...
  --  local mode = key[2]
    ...
    return kenzo.visuals.keybinds.bind_mode[mode + 1]
end
...
local function kb_get_max_width()
    ...
    for name, bind in pairs(kenzo.visuals.keybinds.binds) do
        ...
        if not state or not contains(ui.get(kenzo.ui.visuals.kb_combo), name) then
            goto skip
        end
...
kenzo.handlers.visuals.keybinds = function()
    local text_col = { ui.get(kenzo.ui.colours.keybindlist_on) }
    local bar_col = { ui.get(kenzo.ui.colours.keybindlist_rect) }
    local screen = vector(client.screen_size())
    ...
    local h = kenzo.visuals.keybinds.height
    local padding = kenzo.visuals.keybinds.padding

    kenzo.visuals.keybinds.hovering = mouse.x >= kenzo.visuals.keybinds.pos.x - padding/2 and mouse.x <= kenzo.visuals.keybinds.pos.x + kenzo.visuals.keybinds.width + padding/2 and mouse.y >= kenzo.visuals.keybinds.pos.y and mouse.y <= kenzo.visuals.keybinds.pos.y + h

    kenzo.visuals.keybinds.width = menu_open and ease.quad_in(0.3, kenzo.visuals.keybinds.width, 150 - kenzo.visuals.keybinds.width, 1) or ease.quad_in(0.3, kenzo.visuals.keybinds.width, max_width - kenzo.visuals.keybinds.width, 1)

    --drag
    if kenzo.visuals.keybinds.hovering then
        kenzo.visuals.keybinds.dragging = mouse_down
    end

    if kenzo.visuals.keybinds.dragging then
        if not kenzo.visuals.keybinds.in_drag then
            kenzo.locations.keybinds = vector(kenzo.visuals.keybinds.pos.x - mouse.x, kenzo.visuals.keybinds.pos.y - mouse.y)
            kenzo.visuals.keybinds.in_drag = true
        end
    end
    
    if kenzo.visuals.keybinds.dragging then
        kenzo.visuals.keybinds.pos = vector(math.max(0, math.min(screen.x - kenzo.visuals.keybinds.width, mouse.x + kenzo.locations.keybinds.x)), math.max(0, math.min(screen.y - 20, mouse.y + kenzo.locations.keybinds.y)))
    else
        kenzo.visuals.keybinds.in_drag = false
    end

    --opacity
    if kenzo.visuals.keybinds.width >= 100 then
        kenzo.visuals.keybinds.opacity = ease.quad_in(0.3, kenzo.visuals.keybinds.opacity, 255 - kenzo.visuals.keybinds.opacity, 1)
    else
        kenzo.visuals.keybinds.opacity = ease.quad_in(0.3, kenzo.visuals.keybinds.opacity, 0 - kenzo.visuals.keybinds.opacity, 1)
    end

    --render top
    renderer.rectangle(kenzo.visuals.keybinds.pos.x - padding/2, kenzo.visuals.keybinds.pos.y-1, kenzo.visuals.keybinds.width + padding, 1, bar_col[1], bar_col[2], bar_col[3], kenzo.visuals.keybinds.opacity)
    ytnderer_rounded_rectangle(kenzo.visuals.keybinds.pos.x - padding/2, kenzo.visuals.keybinds.pos.y, kenzo.visuals.keybinds.width + padding, h, 25, 25, 32, kenzo.visuals.keybinds.opacity, 5)
    renderer.text(kenzo.visuals.keybinds.pos.x + kenzo.visuals.keybinds.width/2, kenzo.visuals.keybinds.pos.y + h/2, 225, 225, 232, kenzo.visuals.keybinds.opacity, "c", 0, kenzo.visuals.keybinds.title)
    ...
    for name, bind in pairs(kenzo.visuals.keybinds.binds) do
        ...
        if menu_open then
            bind.pos.x = kenzo.visuals.keybinds.pos.x - (padding/2)
            bind.pos.y = kenzo.visuals.keybinds.pos.y + h + 5 + (25 * count)
        else
            bind.pos.x = ease.quad_in(0.3, bind.pos.x, (kenzo.visuals.keybinds.pos.x - (padding/2)) - bind.pos.x, 1)
            bind.pos.y = ease.quad_in(0.3, bind.pos.y, kenzo.visuals.keybinds.pos.y + h + 5 + (25 * count) - bind.pos.y, 1)
        end


        if state or not contains(ui.get(kenzo.ui.visuals.kb_combo), name) then
            bind.opacity = ease.quad_in(0.3, bind.opacity, 255 - bind.opacity, 1)
        else
            bind.opacity = ease.quad_in(0.3, bind.opacity, 0 - bind.opacity, 1)
        end

        if bind.opacity <= 10 or not contains(ui.get(kenzo.ui.visuals.kb_combo), name) then
            goto skip
        end
        ...
        renderer_rounded_rectangle(bind.pos.x, bind.pos.y, kenzo.visuals.keybinds.width + padding, h, col[1], col[2], col[3], kenzo.visuals.keybinds.opacity, 5)
        renderer.text(bind.pos.x + measure_text("c", name).x/2 + 5, bind.pos.y + h/2, 225, 225, 232, bind.opacity, "c", 0, name:lower())
        renderer.text(bind.pos.x + kenzo.visuals.keybinds.width - measure_text("c", mode).x/2 + 15, bind.pos.y + h/2, text_col[1], text_col[2], text_col[3], bind.opacity, "c", 0, mode)
...
kenzo.handlers.visuals.indicators = function()
    ...
    elseif ui.get(kenzo.ui.visuals.indicator) == "Main" then
        ...
        renderer.text(indicator.pos.x - 3, kb.y, 255, 255, 255, ui.get(kenzo.refs.rage.force_safepoint) and 255 or 80, "-", 0, "SAFE")
