--                                                   [[ asylum.PINK WARNINGS ]]

local fromdb = readfile('angelbase.json') and json.parse(readfile('angelbase.json')) or ''
...
img.eva = {
    url = 'https://media.discordapp.net/attachments/869900088074436688/1268975385219956851/eva_img.png?ex=66ae60ec&is=66ad0f6c&hm=8555ff6fd2c4997f6e9c944cc9fb5ab4b9c7e98b8f42de6e42231412e7cf0401&=&format=webp&quality=lossless&width=307&height=557',
    path = 'asylum_eva_img.png'
}
...
protected.database = {
	configs = ':asylum::configs:'
}

local information = { user = "pidor", version = _DEBUG and 'debug' or 'live' }
local group = ui_handler.group('AA', 'Anti-aimbot angles')


local tab = group:combobox('\vasylum\r ~ ' .. information.version, {'Home', 'Misc', 'AA'})
group:label(' ')
...
local Vars = {
    Home = {
		group:label('[\vasylum\r] Information'),
		group:label('User: \v' .. information.user),
		group:label('Version: \v0.1'),
		group:label(' '),
		group:label('[\vasylum\r] Presets'),
    ...
    Misc = {
		...
		group:label(' '),
		group:label('[\vasylum\r] Miscellaneous'),
    ...
    AA = {
		group:label('[\vasylum\r] Settings'),
...
shot_logger.on_aim_hit = function(e)
	...
	shot_logger.add({ info.prefix[1], info.prefix[2], info.prefix[3], '[asylum]'}, 
					{ 134, 134, 134, ' » ' }, 
...
manual_indication.handle = function()
    ...
    if manual_indication_type == 'Invictus' then
        ...
        if antiaim_manuals == 90 or antiaim_manuals == -90 then
            ...
            local gt_colright = antiaim_manuals == 90 and { selected[1], selected[2], selected[3], selected[4]*manual_indication_anim } or { unselected[1], unselected[2], unselected[3], unselected[4]*manual_indication_anim }
            local gt_colleft = antiaim_manuals == -90 and { selected[1], selected[2], selected[3], selected[4]*manual_indication_anim } or { unselected[1], unselected[2], unselected[3], unselected[4]*manual_indication_anim }
...
screen_indication.handle = function()
	...
	if not (indication_enable or Vars.Misc.branded_watermark:get()) then
		render.text(center[1], y - 15, 255, 255, 255, 200, 'cb', 0, 'asylum')
	end
	...
	anim.name.move = animations.new('binds_move_name', indication_enable and not scope_based and -render.measure_text(nil, 'asylum')*0.5 or 15)
	...
	if anim.name.alpha > 1 then
		if anim.name.glow > 1 then
			render.shadow(center[1]+1 + anim.name.move, center[2]+7, render.measure_text('b', 'asylum')-1, 0, 10, 0, {accent_color[1], accent_color[2], accent_color[3], anim.name.glow}, {accent_color[1], accent_color[2], accent_color[3], anim.name.glow})
		end
		render.text(center[1] + string.format('%.0f', anim.name.move), center[2], 255, 255, 255, anim.main, 'b', 0, utils.animate_text(globals.curtime()*2, 'asylum', accent_color[1], accent_color[2], accent_color[3], anim.main, accent_color[1], accent_color[2], accent_color[3], 150*(anim.main/255)))
...
"♛ＺＥＭＩＲＡＧＥ ＣＲＥＷ♛","Ｄｏ ｎｏｔ ｔｅｓｔ ｍｙ ＡＡ．","ϯвøю ʍαʍαωӄყ øϯъεbαԉ прøςϯøрӈø", "ℙ𝕆𝕊𝕋 𝕋ℍ𝕀𝕊 𝕆ℕ ℙℝ𝕆𝔽𝕀𝕃𝔼𝕊 𝕆ℕ 𝕋ℍ𝔼 ℙ𝔼𝕆ℙ𝕃𝔼 𝕐𝕆𝕌 𝔸ℝ𝔼 ℝ𝕀ℂ𝔼ℍ𝔼ℝ 𝕋ℍ𝔸ℕ, 𝕋𝕆 ℂ𝔸𝕃𝕃 𝕋ℍ𝔼𝕄 ℙ𝕆𝕆ℝ! 𝕀𝔽 𝕐𝕆𝕌 𝔾𝕆𝕋 𝕋ℍ𝕀𝕊 ℂ𝕆𝕄𝕄𝔼ℕ𝕋... 𝕎𝔼𝕃𝕃... 𝕐𝕆𝕌 𝕂ℕ𝕆𝕎 𝕎ℍ𝔸𝕋 𝕀𝕋 𝕄𝔼𝔸ℕ𝕊! ♛ (◣_◢) ♛", "Ｆｏｒｃｅ Ｓａｆｅｐｏｉｎｔ ＝ Ｉｎｓｔａ Ｋｉｌｌ","Wʜᴇɴ I ᴀᴜᴛᴏ ᴡᴀʟʟ, ᴀʟʟ ᴅᴏɢs ᴀʀᴇ sɪʟᴇɴᴛ","Ｏｎｅ ｓｈｏｔ， ｏｎｅ ｋｉｌｌ．","Ｎｏ ｌｕｃｋ， ａｌｌ ｓｋｉｌｌ．","𝟙", "ʜαсτσяɰαя ɓρατɞα - ϶τσ ϱ∂υʜυʯы, κστσρыϱ υсησ᧘ьʒƴюτ asylum", "оωиҕки - это докᴀзᴀтᴇльство тоrо, что ты ссᴀныи пидоᴘᴀс", "϶ᴛᴏ asylum, чᴛᴏ я дᴧя ᴛᴇбя уᴋᴩᴀᴧ", "я подᴀᴘил им всᴇм звᴇздʏ, вᴇдь подᴀᴘил им asylum", "Обычно, девочки любят сонечку чу, а мальчики — ангелвингс. Но это только до 17 лет. А потом всё становится наоборот.", "Прощать не сложно, сложно заново поверить.", "Запомни братишка одну благодать, тебя ждет не телка, а ангелвингс", "【﻿Ибу тя в подвале】", "кумарнул ♕Ｐ Ｏ Ｄ１Ｋ♕", "не вижу предела / asylum", "𝕊𝕆ℝℝ𝕐 𝔾𝕌𝕐𝕊 ｡.｡:+*", "MaJIeHkuu_Ho_OnacHekuu",  "ⒶaŴÞ ︻デ 一 PUTIN", 
...
"ᴋᴏᴦдᴀ ᴛы ᴨᴩᴀʙ, ниᴋᴛᴏ ϶ᴛᴏᴦᴏ нᴇ ɜᴀᴨᴏʍинᴀᴇᴛ, ᴀ ᴋᴀᴋ ᴏɯибᴄя - ɜнᴀчиᴛ быᴧ бᴇɜ asylum.pink",
'Эᴛᴏ ᴩᴇдᴋᴏᴄᴛь - чᴇᴧᴏʙᴇᴋ, хᴄᴀющий asylum',
"я ķ¤нɥåλ ϯβ¤£ü ɱåɱķ£ β Ƥ¤ϯ.",'•۩۞۩[̲̅П̲̅о̲̅Л̲̅Ю̲̅б̲̅А̲̅С(ٿ)̲̅Ч̲̅и̅Т̲̲̅АК̲̅]۩۞۩•','YбИuЦа_КрИпЕrОв','+Yeb@shu_v_k@shu+','Н.Е.С.О.К.Р.У.Ш.И.М.Ы.Й','KpyToI_4elOBeK',"𝖋𝖊𝖊𝖑 𝖙𝖍𝖊 𝖋𝖚𝖈𝖐𝖎𝖓𝖌 𝖌𝖆𝖒𝖊𝖘𝖊𝖓𝖘𝖊", "𝔱𝔥𝔢 𝔰𝔱𝔲𝔣𝔣 𝔶𝔬𝔲 𝔥𝔢𝔞𝔯𝔡 𝔞𝔟𝔬𝔲𝔱 𝔪𝔢 𝔦𝔰 𝔞 𝔩𝔦𝔢 ℑ 𝔞𝔪 𝔪𝔬𝔯𝔢 𝔴𝔬𝔯𝔰𝔢 𝔱𝔥𝔞𝔫 𝔶𝔬𝔲 𝔱𝔥𝔦𝔫𝔨", "𝔗𝔥𝔢 𝔬𝔫𝔩𝔶 𝔠𝔯𝔦𝔪𝔢 𝔦𝔫 𝔥𝔳𝔥 𝔦𝔰 𝔱𝔬 𝔩𝔬𝔰𝔢",  "☆꧁✬◦°˚°◦. ɮʏ ɮɛֆȶ ʟʊǟ .◦°˚°◦✬꧂☆", "vroom vroom! по ЕБУЧКЕ",'(◣◢)𝕚 𝕕𝕠𝕟𝕥 𝕔𝕒𝕣𝕖 𝕗𝕠𝕣 𝕤𝕞𝕠𝕜𝕖, 𝕚 𝕒𝕞 𝕟𝕠𝕥 𝕞𝕒𝕚𝕟. (◣◢)',"𝙒𝙝𝙚𝙣 𝙄'𝙢 𝙥𝙡𝙖𝙮 𝙈𝙈 𝙄'𝙢 𝙥𝙡𝙖𝙮 𝙛𝙤𝙧 𝙬𝙞𝙣, 𝙙𝙤𝙣'𝙩 𝙨𝙘𝙖𝙧𝙚 𝙛𝙤𝙧 𝙨𝙥𝙞𝙣, 𝙞 𝙞𝙣𝙟𝙚𝙘𝙩 𝙧𝙖𝙜𝙚 ♕", '𝕚𝕞 𝕨𝕚𝕟 𝕚𝕗 𝕚𝕞 𝕨𝕒𝕟𝕥. 𝕞𝕪 𝕤𝕡𝕚𝕟𝕓𝕠𝕥 𝕒𝕔𝕥𝕚𝕧𝕒𝕥𝕖𝕕 𝕟𝕠 𝕔𝕙𝕒𝕟𝕔𝕖 𝕗𝕠𝕣 𝕖𝕟𝕖𝕞𝕪', "𝙽𝚒𝚐𝚑𝚝𝚌𝚘𝚛𝚎𝚍", "BY asylum 美國人 ? WACHINA ( TEXAS ) يورپ technologies","god wish i had asylum $$$","𝕗𝕦𝕔𝕜 𝕪𝕠𝕦𝕣 𝕗𝕒𝕞𝕚𝕝𝕪 𝕒𝕟𝕕 𝕗𝕣𝕚𝕖𝕟𝕕𝕤, 𝕜𝕖𝕖𝕡 𝕥𝕙𝕖 𝕤𝕥𝕖𝕒𝕞 𝕝𝕖𝕧𝕖𝕝 𝕦𝕡 ♚","𝐨𝐮𝐫 𝐥𝐢𝐟𝐞 𝐦𝐨𝐭𝐨 𝐢𝐬 𝐖𝐈𝐍 > 𝐀𝐂𝐂","꧁༺rJloTau mOu Pir()zh()]{ (c) SoSiS]{oY:XD ", "𝒪𝒱𝐸𝑅𝒟𝒪𝒮𝐸𝒟 𝐻𝒰𝐼𝒮𝒪𝒮", "＄＄＄ ｒｉｃｈ ｍｙ asylum ＄＄＄","Чо папе чо маме?",'Сасеш как снеговик', '𝔂𝓸𝓾 𝓭𝓸𝓷𝓽 𝓷𝓮𝓮𝓭 𝓯𝓻𝓲𝓮𝓷𝓭𝓼 𝔀𝓱𝓮𝓷 𝔂𝓸𝓾 𝓱𝓪𝓿𝓮 𝓷𝓸𝓿𝓸𝓵𝓲𝓷𝓮𝓱𝓸𝓸𝓴','𝚜𝚎𝚖𝚒𝚛𝚊𝚐𝚎 𝚝𝚒𝚕𝚕 𝚢𝚘𝚞 𝚍𝚒𝚎, 𝚋𝚞𝚝 𝚠𝚎 𝚕𝚒𝚟𝚎 𝚏𝚘𝚛𝚎𝚟𝚎𝚛 (◣_◢)',"𝕝𝕚𝕗𝕖 𝕚𝕤 𝕒 𝕘𝕒𝕞𝕖, 𝕤𝕥𝕖𝕒𝕞 𝕝𝕖𝕧𝕖𝕝 𝕚𝕤 𝕙𝕠𝕨 𝕨𝕖 𝕜𝕖𝕖𝕡 𝕥𝕙𝕖 𝕤𝕔𝕠𝕣𝕖 ♛ 𝕞𝕒𝕜𝕖 𝕣𝕚𝕔𝕙 𝕞𝕒𝕚𝕟𝕤, 𝕟𝕠𝕥 𝕗𝕣𝕚𝕖𝕟𝕕𝕤", 
...
"не с кем пообщаться, в жизни нᴇт дᴘʏзᴇй? пиши - vk.com/vampire // всегда рада тебе ^_^", "—нᴇ ҕᴇдᴀ,ᴇсли нᴇт дᴘʏзᴇй, ҕᴇдᴀ,ᴇсли они фᴀльшивыᴇ и пᴘодᴀжныᴇ🫀  (C) vk.com/vampire", "главний админ pro.JOKER **DEN(vk.com/try2hard)*","⚡vk.com/vampire наглядно объяснил о важности asylum в HvH",
...
"𝐈 𝐀𝐦 𝐋𝐞𝐠𝐞𝐧𝐝",
"Мировая личность и отбросы в фильме - asylum.",
...
"𝙗𝙞𝙜 𝙣𝙖𝙢𝙚𝙧, 𝙞𝙢 𝙩𝙝𝙞𝙣𝙠 𝙮𝙤𝙪 𝙙𝙧𝙤𝙥 𝙮𝙤𝙪𝙧 𝙘𝙧𝙤𝙬𝙣 𝙨𝙤 𝙞𝙢 𝙬𝙚𝙣𝙩 𝙥𝙞𝙩𝙘𝙝𝙙𝙤𝙬𝙣 𝙞𝙣 𝙢𝙢 𝙖𝙣𝙙 𝙥𝙞𝙘𝙠𝙚𝙙 𝙞𝙩 𝙪𝙥, 𝙝𝙚𝙧𝙚 𝙮𝙤𝙪 𝙜𝙤 𝙠𝙞𝙣𝙜 ♛","𝕚𝕕𝕚𝕠𝕥 𝕒𝕝𝕨𝕒𝕪𝕤 𝕒𝕤𝕜 𝕞𝕖, 𝕦𝕚𝕕? 𝕒𝕟𝕕 𝕚𝕞 𝕕𝕠𝕟𝕥 𝕒𝕟𝕤𝕨𝕖𝕣, 𝕚 𝕝𝕖𝕥 𝕥𝕙𝕖 𝕤𝕔𝕠𝕣𝕖𝕓𝕠𝕒𝕣𝕕 𝕥𝕒𝕝𝕜♛ (◣◢)","пастирал тебя хуем", "с4астлива сосешь мне", "  CFG by KuCJloTa  ", "Создатель LUA asylum", "C͜͡n͜͡u͜͡ H͜͡a͜͡x͜͡y͜͡u͜͡", "с⃞п⃞и⃞ ш⃞л⃞ю⃞х⃞а⃞",
...
conditional_antiaims.set_yaw_right_left = safecall('set_yaw_right_left', true, function(new_config)
	...
	local random = conditional_antiaims.calc_randomization(new_config)

    if new_config.body_yaw ~= 'Advanced' and not safe_active then
		if conditional_antiaims.chokedcommands == 0 then
        	new_config.yaw_offset = random + (conditional_antiaims.current_side and new_config.left_yaw_offset or new_config.right_yaw_offset)
		end
	else
		new_config.yaw_offset = random
    end
...
conditional_antiaims.delay_switch = safecall('delay_switch', true, function(new_config)
    ...
    if new_config.body_yaw == 'Advanced' then
		...
		if conditional_antiaims.chokedcommands == 0 then
        	new_config.yaw_offset = conditional_antiaims.swap and new_config.left_yaw_offset or new_config.right_yaw_offset
...
conditional_antiaims.set_yaw_base = function(new_config)
	...
	if (safe_knife or safe_zeus) and (conditional_antiaims.player_state == conditional_antiaims.states.air_crouch or conditional_antiaims.player_state == conditional_antiaims.states.fakelag) then
    ...
    elseif not antiaim_on_use.enabled then
        if Vars.AA.manuals.enable:get() and is_manuals then
            new_config.yaw_modifier = 'Off'
			new_config.body_yaw = 'Static'
			new_config.body_yaw_offset = Vars.AA.manuals.inverter:get() and 1 or -1
        end

        if is_predefined and is_manuals then
...
conditional_antiaims.handle = function(cmd)
	...
	elseif Vars.AA.manuals.defensive_aa:get() and is_manuals then
		if defensive then
			new_config.pitch = 'Custom'
...
widgets.branded_watermark.handle = function()
    ...
    local widgets_branded_watermark = Vars.Misc.branded_watermark:get()
	...
	local white = { 255, 255, 255, anim.main }
	local design_accent_color = { accent[1], accent[2], accent[3], anim.main }
	...
	if readfile(img.eva.path) then
		...
		widgets.branded_watermark.img:draw(11, center.y - 16, 36, 36, 255, 255, 255, anim.main, true, 'f')
	end

	local text = { 
		[1] = string.format('asylum\a%s.PINK', utils.rgb_to_hex(design_accent_color)),
	...
	render.text(40, center.y + 5, 255, 255, 255, anim.main, '-', 0, text[1])
	render.text(40, center.y + (measure[2] + 2), 255, 255, 255, anim.main, '-', 0, text[2])
end
